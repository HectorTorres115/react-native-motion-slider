/**
 * @author Héctor Torres - https://github.com/HectorTorres115
 */
 import React, { Component } from 'react';
 import { View, Text, Dimensions, Animated, Easing, PanResponder, StyleSheet } from 'react-native';
 ...
 class Slider extends Component {
	 ...
	 shoulApearText (){
		 if(this.state.value < this.props.max && this.state.value == 0){
			 return (
				 <Text style = {newStyles.sliderText}>{this.props.firstMessage}</Text>
			 )
		 } else if (this.state.value !== 0 && this.state.value < this.props.max){
			 return (
				 <Text style = {newStyles.sliderText}>{this.props.secondMessage}</Text>
			 )
		 } else if (this.state.value == this.props.max){
			 return <Text style = {newStyles.sliderText}>{this.props.finalMessage}</Text>
		 } else {
			 return null
		 }
	 }
